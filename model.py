...
def compute_ukf_loglikelihood(
    ...
    def process_batch(batch_states, batch_inputs, signal_category):
        ...
        for states, inputs in zip(batch_states, batch_inputs):
            ...
            ukf.x = np.atleast_1d(states[0])   # Initial state from observation (or 0)
...
# Alias for compatibility if needed, though mostly redundant now
class StratifiedLinear_old(StratifiedLinear):
    pass

class StratifiedLinear_kalman(StratifiedLinear):
    pass
...
class StratifiedNonLinear1(LEAD_abstract):
    ...
    """
    _param_names = LEAD_abstract._param_names + [f'w{i}' for i in range(7)] + ['gain', 'threshold']
